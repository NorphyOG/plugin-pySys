...
- **Primary modules:** `plugin.py` handles the Qt view with Equalizer & Recorder tabs; `equalizer.py` (if present) encapsulates DSP filters; `recorder.py` orchestrates `sounddevice` streams and writes metadata via `mutagen`.
...
- **UI & UX:** Grid/table views support lazy loading and chunked updates (see `docs/system-tools-plan.md` performance table). Maintain responsive selection syncing and auto-preview refresh when altering caching logic.
...
- **Future panels:** `docs/system-tools-plan.md` tracks upcoming Disk Integrity Monitor, Image compression, and temp cleaner tasks. Document new features there as you finish or schedule them.
...
- **Scheduling & timers:** Auto-start logic uses `_restore_dashboard_state()` on launch. Reuse patterns when adding auto-refresh toggles or scheduled tasks (for example, backup timers).
